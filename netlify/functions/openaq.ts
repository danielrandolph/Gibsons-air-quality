...
// Bonus/cross-check source only — not included in the blended number (thin, inconsistent
// coverage in rural BC since OpenAQ just aggregates other networks rather than adding sensors).
export const handler: Handler = async () => {
  const apiKey = process.env.OPENAQ_API_KEY;
  if (!apiKey) {
    return json({ name: 'OpenAQ', available: false, error: 'No API key configured', baseWeight: 0, includeInBlend: false });
  }
  ...
  try {
    ...
    if (!locRes.ok) {
      return json({ name: 'OpenAQ', available: false, error: `HTTP ${locRes.status}`, baseWeight: 0, includeInBlend: false });
    }
    ...
    if (!location) {
      return json({ name: 'OpenAQ', available: false, error: 'No nearby stations', baseWeight: 0, includeInBlend: false });
    }
    ...
    return json({ name: 'OpenAQ', available: false, error: err.message ?? 'Fetch failed', baseWeight: 0, includeInBlend: false });
  }
};
